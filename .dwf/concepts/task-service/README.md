...
- A task page reads at most one row beyond its limit, projects only application-required fields, and does not trigger one follow-up query per returned task.
...
- On the representative Neon seed, warm 20-record task queries satisfy the agreed database-execution target without a full table scan.
