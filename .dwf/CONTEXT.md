...
- `.github/workflows/ci.yml` runs Quality and Harness on main pushes/pull requests without deployment. `.github/workflows/deploy-preview.yml` is manual only. T-22 hosted proof remains incomplete; PR #30 records command repairs and a cleaned-up attempt whose first Vercel deployment was classified Production. On 2026-09-14 the owner resolved that with a placeholder Production deployment, a separate Neon Production project was provisioned for T-23, and the adapter gained a project preflight plus team-scoped identity validation. A hosted run still needs owner authorization. No Production release workflow exists.
