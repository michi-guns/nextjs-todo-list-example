...
| [TST-RELEASE-001](#tst-release-001)         | An approved exact-ref release is migrated, deployed, smoked, and recorded            | Protected release rehearsal and Production evidence                  | T-21.5, T-23, T-24                                                 | `blocked`  |
...
The authenticated Vercel CLI cannot create the required project-scoped token
(`403: Cannot create tokens for this app.`). The browser form awaits the
owner confirmation required by that tool before creating new access and
storing `VERCEL_TOKEN` in GitHub `production`. Status is now `blocked` at
this named prerequisite; no executable release implementation, migration or
deployment has started. See the [preflight evidence](../../docs/agentforge/evidence/2026-09-16-production-release-preflight.md)
