...
function renderListForm(state, compact = false) {
  ...
  return `<form class="list-form surface-card" data-form="list" ${compact ? 'data-compact="true"' : ""}>
    ...
    <input class="field" id="list-name" name="name" maxlength="80" aria-required="true" value="${escapeHtml(isCreate ? "" : (currentList(state)?.name ?? ""))}" placeholder="${isCreate ? "Name your list" : "List name"}" />
