...
| Future `db/schema/logging.ts`, selected settings writer and `src/test/logging-settings.integration.test.ts` | Versioned policy transition using the same environment-local snapshot; add a forward migration only if the stored representation requires it.              |
...
| ----------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Completed T-26.1 through T-26.3 and execution instruction                           | Required to implement diagnostics against the actual logger contracts. OD-026 blocks its upstream writer; this plan does not choose that interface.                               |
