...
- `scripts/README.md` requires a self-contained script directory with a thin CLI, reusable core, one package command, and focused tests where reusable logic warrants them. Existing `scripts/verify-neon.mjs` is a read-only connection smoke and remains unchanged. The performance CLI invokes `neon connection-string development` itself so the branch endpoint is authoritative rather than caller-labeled.
...
- The agent-owned Neon `development` branch is ready and has a known expiration date. Preflight must verify its identity and readiness before any mutation; the script must fail closed when the independent Neon CLI endpoint is unavailable or does not match the supplied URL.

## Architecture and implementation slices

1. Add `scripts/verify-neon-performance/` with a thin `cli.mjs`, reusable `core.mjs`, and focused core tests if parsing/plan assertions are factored into testable functions. Add exactly one `pnpm neon:performance` command. Before connecting or mutating, the CLI obtains the direct `development` connection string by invoking the authenticated Neon CLI itself; an optional `DATABASE_URL` is accepted only when its host matches that authoritative endpoint. It never reads or persists `.env.local` secrets by default.
...
- The CLI refuses an unavailable authoritative development endpoint, non-PostgreSQL targets, a supplied host that does not match the independent development-branch preflight, unexpected database identity, or an unrecognized synthetic-prefix configuration before mutating data.
...
- Run the script with the authenticated Neon CLI obtaining the ephemeral direct URL from `neon connection-string development` and verify the independent endpoint guard, seed counts, plan/index assertions, cursor checks, and timing report. Also verify that a default-branch URL supplied through `DATABASE_URL` is rejected before mutation.
