...
The accepted plan [`2026-08-31-t-15-playwright-harness.md`](../plans/2026-08-31-t-15-playwright-harness.md) is implemented on `task/T-15-playwright-harness`. The deterministic local Playwright lifecycle owns one harness-provided PostgreSQL 18 container, the committed migration chain, six verified scenario users, fixed list/task fixtures, a temporary local magic-link mailbox, a dedicated `http://127.0.0.1:3100` Next.js server, serial Chromium journeys, and cleanup on pass or failure. The application-facing landing fixture is enabled only for `PLAYWRIGHT_E2E=true` outside production; the normal Sanity path remains unchanged. The default project list is Chromium-only; `pnpm test:e2e:cross-browser` opts into Firefox/WebKit through `PLAYWRIGHT_CROSS_BROWSER=true` without reusing an unknown server or external database.

Evidence at implementation tip `c606d54`: `pnpm test` passes 23 files/120 tests; `pnpm test:integration` passes 6 files/23 tests against one disposable `postgres:18-alpine` Testcontainer; `pnpm test:e2e` passes all 7 Chromium journeys; project selection lists 21 tests (7 per Chromium/Firefox/WebKit) under the opt-in switch; typecheck, lint (one pre-existing `Geist` warning), build, scoped Prettier, Drizzle check, and diff checks pass. The browser journeys cover deterministic landing, password session/sign-out, list/task/status mutation, magic-link request/read/consume, two-user privacy isolation, pagination/filtering, and dashboard skip-link/next-Tab behavior. The runner review at `c606d54` returned **No actionable findings**. `TST-AUTH-001`, `TST-AUTH-002`, `TST-AUTH-003`, `TST-UI-001`, `TST-E2E-001`, `TST-E2E-002`, and `TST-E2E-003` are verified; `TST-HARNESS-001` remains partial only because a live Docker-daemon outage was not intentionally induced.

The next closeout action is to commit and review the reconciled TODO/testing/checkpoint metadata, open and merge T-15, pull `main` fast-forward, and recompute dependencies. T-17 then becomes the next safely unblocked task.
