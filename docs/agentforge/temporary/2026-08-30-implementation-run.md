...
The current TODO graph marks T-09A complete and PR #15 merged as `b88f377`, T-09B complete on `main` at `994bca0`, T-16 complete in TODO after final reviewed metadata tip `85d3998` and PR #16 merge commit `5e40bfd` (the hosted evidence itself ran against implementation commit `7837a69`), T-10 complete at reviewed code tip `a25461b`, T-11 complete on `origin/main` at `aae8ec2`, T-12A complete and merged as PR #17, and T-15 complete at corrected implementation tip `6f841a3` with final closeout review tip `f73223c`, merged as PR #18 (`3c4b0d5`). T-17 is the final safely unblocked task and its documentation slices are now complete; final metadata review and closeout remain. No later implementation task is currently unblocked.

## T-12A closeout checkpoint

The accepted plan [`2026-08-31-t-12a-ui-audit.md`](../plans/2026-08-31-t-12a-ui-audit.md) completed on branch `task/T-12A-ui-audit` and merged as PR [#17](https://github.com/michi-guns/nextjs-todo-list-example/pull/17) (`df08ed4`). The reviewed implementation tip `a3eed2c` adds a shared focus-visible skip link to the public/auth/dashboard shells and hides the decorative landing preview from the accessibility tree. The first review of `7c3d320` found that the landing target included header navigation; `a3eed2c` moves the target to the post-header content region so the next Tab reaches `Get started`. The fresh review of `a3eed2c` returned **No actionable findings**. Closeout metadata began at `5dc6bd6`; its stale-state finding was corrected in `98e612e`, and the final closeout was completed before the merge.
...
The accepted plan [`2026-08-31-t-15-playwright-harness.md`](../plans/2026-08-31-t-15-playwright-harness.md) is complete on `task/T-15-playwright-harness`. The deterministic local Playwright lifecycle owns one harness-provided PostgreSQL 18 container, the committed migration chain, six verified scenario users, a listless core fixture whose first dashboard load provisions the ordinary `Inbox`, fixed list/task fixtures for the other scenarios, a temporary local magic-link mailbox, a dedicated `http://127.0.0.1:3100` Next.js server, serial Chromium journeys, and cleanup on pass or failure. The application-facing landing fixture is enabled only for `PLAYWRIGHT_E2E=true` outside production; the normal Sanity path remains unchanged. The default project list is Chromium-only; `pnpm test:e2e:cross-browser` opts into Firefox/WebKit through `PLAYWRIGHT_CROSS_BROWSER=true` without reusing an unknown server or external database.

Evidence for the corrected local run: the seed-plan tests pass 4/4; `pnpm test` passes 23 files/121 tests; `pnpm test:integration` passes 6 files/23 tests against one disposable `postgres:18-alpine` Testcontainer; `pnpm test:e2e` passes all 7 Chromium journeys, including automatic core `Inbox` provisioning; project selection lists 21 tests (7 per Chromium/Firefox/WebKit) under the opt-in switch; typecheck, lint (one pre-existing `Geist` warning), build, scoped Prettier, Drizzle check, and diff checks pass. The browser journeys cover deterministic landing, password session/sign-out, list/task/status mutation, magic-link request/read/consume, two-user privacy isolation, pagination/filtering, and dashboard skip-link/next-Tab behavior. Fresh GPT-5.6-Sol reviews of corrected implementation tip `6f841a3` and final closeout tip `f73223c` returned **No actionable findings**; PR #18 merged as `3c4b0d5`. `TST-AUTH-001`, `TST-AUTH-002`, `TST-AUTH-003`, `TST-UI-001`, `TST-E2E-001`, `TST-E2E-002`, and `TST-E2E-003` are verified by the corrected execution evidence; `TST-HARNESS-001` remains partial only because a live Docker-daemon outage was not intentionally induced.

The T-15 closeout is complete. The next action is to plan and implement T-17 from the synchronized `main`; its dependencies are satisfied, and the remaining baseline obligations are README/setup documentation, implementation-specific command/recovery documentation, final gates, and SPEC definition-of-done review.

## T-17 closeout checkpoint

The accepted plan [`2026-08-31-t-17-documentation-quality.md`](../plans/2026-08-31-t-17-documentation-quality.md) is complete as documentation and evidence work only. `README.md` now gives a quick start, exact environment categories, direct-versus-pooled database guidance, Better Auth mailbox behavior, migration/snapshot context, commands, architecture links, and recovery links. [`docs/runbooks/local-development-and-verification.md`](../../runbooks/local-development-and-verification.md) documents Drizzle inspection/application, the environment-gated migration policy, disposable Testcontainers and Playwright lifecycles, the read-only Sanity smoke, and failure/recovery boundaries. No application source, dependency, schema, migration, provider, or deployment state changed.

Current local evidence: `pnpm typecheck` passes; `pnpm lint` has zero errors and only the pre-existing `app/layout.tsx:1:10` unused `Geist` warning; `pnpm test` passes 23 files/121 tests; `pnpm test:integration` passes 6 files/23 tests against one disposable `postgres:18-alpine` Testcontainer; `pnpm test:e2e` passes all 7 serial Chromium journeys; `pnpm sanity:smoke` passes the published singleton read/validate/map smoke; `pnpm exec drizzle-kit check --config drizzle.config.ts` passes; `pnpm build` passes; changed-file Prettier, relative-link, and `git diff --check` checks pass. `PLAYWRIGHT_CROSS_BROWSER=true pnpm exec playwright test --list` exposes 21 tests across Chromium, Firefox, and WebKit; the optional Firefox/WebKit execution was not run.

The active testing ledger remains honest: `TST-MIGRATION-001` is blocked pending an explicitly authorized realignment of the agent-owned Neon development branch, `TST-HARNESS-001` is partial because a live Docker-daemon outage was not observed, `TST-CONCURRENCY-001` is verified by the application and PostgreSQL commit-ordering/disjoint-field evidence, and `TST-LANDING-003` is partial pending deployed webhook delivery. Other baseline contracts retain their recorded verified status. The SPEC definition-of-done review confirms the local baseline evidence; production/deployment-only and deferred preview obligations remain explicitly out of scope.

T-17.1 and T-17.2 each received a fresh GPT-5.6-Sol medium review. The first README review found the mailbox requirement and the runbook review found the Sanity-smoke and Neon-direct-URL wording issues; all were fixed and the corrected documentation was re-reviewed with no actionable findings. T-17.3 final metadata review found and corrected historical TODO status wording, then returned no actionable findings. T-17 is complete at this reviewed documentation tip, and no safely unblocked task remains.
