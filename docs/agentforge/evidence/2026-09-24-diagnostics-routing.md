...
limit is documented in code and here rather than engineered away: after
the first stack read, a message rewritten to text the cached header still
ends with (its leading line or lines, a `: `-delimited tail of its first line,
or empty when that first line was empty) cannot be told apart from an honest
header. A fourth review confirmed this scope. Application
