...
| `health`           | `health.database.unavailable`, `health.cms.unavailable`                                                                                                  | Local `warn` when a dependency probe fails, outcome `failed` or `timeout`; no issue report, the uptime monitor owns alerting. See the [operations runbook](operations.md)                               |
