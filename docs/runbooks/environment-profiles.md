...
| `NEXT_PUBLIC_SANITY_EDITORIAL_PREVIEW_ENABLED` | Optional           | Editorial Draft Mode capability; `true` only for Local/Development/Production on `production`; Preview must not enable it |
...
- Preview and Production require `DATABASE_ENDPOINT_HOST`, the endpoint the
  delivery adapter observed; the pooled runtime host must belong to it. A
  branch label alone is not proof of the target. Other profiles check it
  only when present.
