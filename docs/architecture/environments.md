...
| Preview     | Intended ephemeral Vercel deployment, expiring Neon child of Development | Controlled verified account, no sends; Sanity `preview`  | Tooling exists; hosted proof blocked by Vercel first-deployment prerequisite |
...
proof. The current Vercel adapter needs first-deployment preflight and identity
lookup repair before another run. See [Preview delivery](../runbooks/preview-delivery.md).
