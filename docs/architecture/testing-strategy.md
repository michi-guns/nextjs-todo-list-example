...
Use Playwright for the core sign-in → create list → create task → change status → sign-out journey. For authentication-link verification, explicitly enable the local/test file-backed mailbox, clear it before the test, request the relevant link, read the captured URL, and visit it. Password sign-up first requires consuming the captured email-verification link for password access; a verified account may then use either password or magic-link sign-in without losing its password credential. If a magic link is consumed before email verification, Better Auth deliberately revokes the unproven password credential as its pre-hijacking defense. The temporary mailbox is gitignored and unavailable outside local/test mode.
