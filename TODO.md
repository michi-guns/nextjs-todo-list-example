...
- [ ] Hosted functional Preview proof remains incomplete. The Vercel project and Preview secrets were supplied for the 2026-09-09 attempt; the resulting first deployment was classified Production and was deleted, and guarded Neon cleanup succeeded. The first-deployment prerequisite was resolved on 2026-09-14 by the placeholder Production deployment recorded in the [Preview runbook](docs/runbooks/preview-delivery.md). Retry only with the identity repairs above and an explicit owner authorization for the hosted run. Vercel Git auto-deploy must stay disabled.
