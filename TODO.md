...
- [x] T-23.2 — `scripts/deploy/production/ref.ts` resolves a tag/full SHA to `ResolvedDeliveryRef`, checks reviewed-main ancestry and clean matching checkout, and requires exact-SHA main-push `ci.yml` Quality/Harness success. The safe subprocess boundary captures errors without exposing arguments/stderr. Focused Vitest passes 28 cases across two files; typecheck and changed-file lint pass. A read-only real Git/GitHub check at `ad16b60209863ad36dfcecbb3be6de1fc7569bb8` confirmed successful CI run `35105163127`, attempt 1. No protected access or mutation was performed. Fresh independent review gates this implementation unit; workflow wiring remains T-23.4.
