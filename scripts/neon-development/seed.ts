...
import {
  NEON_DEVELOPMENT_BEHAVIOR_LIST_COUNT,
...
const ORDINARY_TASKS = [
  { title: "Welcome to development", status: "todo" as const, notes: null },
  {
    title: "Try the durable Neon target",
    status: "in_progress" as const,
    notes: "This row is synthetic development seed data.",
  },
] as const

export async function seedNeonDevelopment(
  profile: EnvironmentProfile,
  mode: "ordinary" | "behavior"
): Promise<void> {
  if (profile.appEnv !== "development") {
...
async function ensureVerifiedUser(
  ...
  const response = await auth.handler(
    new Request(`${origin}/api/auth/sign-up/email`, {
      ...
      body: JSON.stringify({
        name: NEON_DEVELOPMENT_SEED_USER.name,
        email: NEON_DEVELOPMENT_SEED_USER.email,
        password: NEON_DEVELOPMENT_SEED_USER.password,
      }),
    })
  )
  if (!response.ok) {
    throw new Error("Unable to create the development synthetic user")
  }

  const userId = readUserId(await response.json())
  if (!userId) {
    throw new Error("Development synthetic user response was invalid")
  }

  const message = await mailbox.readLatestMagicLink(
    NEON_DEVELOPMENT_SEED_USER.email
  )
  if (!message || !message.url.includes("/verify-email")) {
    throw new Error(
      "Development synthetic user verification message was not captured"
    )
  }
  ...
  if (verificationResponse.status < 300 || verificationResponse.status >= 400) {
    throw new Error("Unable to verify the development synthetic user")
  }
  ...
  try {
    ...
    const list = await client.query<{ id: string }>(
      `INSERT INTO lists (user_id, name)
       VALUES ($1, $2)
       RETURNING id`,
      [userId, NEON_DEVELOPMENT_SEED_USER.listName]
    )
    const listId = list.rows[0]?.id
    if (!listId) {
      throw new Error("Development synthetic list was not created")
    }

    for (const task of ORDINARY_TASKS) {
      await client.query(
    ...
    if (mode === "behavior") {
      for (
        let index = 1;
        index <= NEON_DEVELOPMENT_BEHAVIOR_LIST_COUNT;
        index += 1
      ) {
        await client.query(
          `INSERT INTO lists (user_id, name)
           VALUES ($1, $2)`,
          [userId, `Behavior list ${String(index).padStart(2, "0")}`]
        )
      }
      for (
        let index = 1;
        index <= NEON_DEVELOPMENT_BEHAVIOR_TASK_COUNT;
        index += 1
      ) {
        await client.query(
          `INSERT INTO tasks (list_id, user_id, title, notes, status)
          ...
          [
            listId,
            userId,
            `Behavior task ${String(index).padStart(2, "0")}`,
            null,
            "todo",
          ]
        )
